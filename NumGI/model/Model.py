...
class TransformerNet(nn.Module):
    """Main Transformer model we will be using for the project.
    ...
    def forward(self, src, tgt, mask_in, mask_out):
        src, tgt = self.enc_embedding(src).permute(1, 0, 2), self.dec_embedding(tgt).permute(
            1, 0, 2
        )
        ...
        transformer_out = self.decoder(
            tgt, memory, tgt_mask=self.tgt_mask, tgt_key_padding_mask=mask_out
        )
